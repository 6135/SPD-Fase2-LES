...
class TestForms(TestCase):
    def setUp(self):
        ...
        self.menu = create_menu(
            campus=self.campus, horario=self.horarioAlmoco, diaaberto=self.diaaberto
        )
    ...
    def test_TemaForm(self):
        form = TemaForm(data={"tema": "Informatica"})
    ...
    def test_EdificioForm(self):
        ...
        formEdificio = EdificioForm(
            data={"nome": "C1", "campus": self.campus, "image": "bruv.png"}
        )
        ...
        formEdificio = EdificioForm(
            data={"nome": "C1", "campus": self.campus.nome, "image": "bruv.png"}
        )
    ...
    def test_transporteUniversitarioForm(self):
        form = transporteUniversitarioForm(data={"capacidade": 1})
        self.assertTrue(form.is_valid())
    ...
    def test_menuForm(self):
        ...
        form = menuForm(data={"diaaberto": None, "campus": None, "dia": None})

        self.assertFalse(form.is_valid())
        self.assertEquals(
            len(form.errors), 4
        )  # 4 para o horario que e dado automaticamente
