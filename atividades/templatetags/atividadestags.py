...
def has_conflict(value, activity_id):
    for conf in value:
        if conf.atividade1.atividadeid.id == activity_id:
            return mark_safe(
                '<a type="button" onclick="alert.render(\'A Atividade tem conflitos, tem a certeza que desenja proceder?\','
                + "/atividades/validaratividade/"
                + str(activity_id)
                + "/"
                + "0"
                + ');"  class=" button is-success" style="margin-right: 10px;" ><span class="icon is-small"><i class="mdi mdi-check"></i></span><span>Aceitar</span></a>'
            )
    return mark_safe(
        '<a type="button" href=" '
        + "/atividades/validaratividade/"
