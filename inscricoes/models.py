...
class Inscricao(models.Model):
    ...
    meio_transporte = models.CharField(max_length=40, choices=MEIO_TRANSPORTE_CHOICES)
    ...
    def get_departamentos(self):
        return {
            inscricaosessao.sessao.atividadeid.get_departamento()
            for inscricaosessao in self.inscricaosessao_set.all()
        }
    ...
    def get_dias(self):
        inscricao_sessoes = Inscricaosessao.objects.filter(inscricao=self).order_by(
            "sessao__dia"
        )
    ...
    def get_horarios(self, dia):
        inscricao_sessoes = Inscricaosessao.objects.filter(
            inscricao=self, sessao__dia=dia
        ).order_by("sessao__horarioid__inicio")
        horarios = []
        horarios.append(
            {
                "key": str(inscricao_sessoes.first().sessao.horarioid.inicio),
        ...
        for sessao in inscricao_sessoes:
            if sessao.sessao.horarioid.fim not in horarios:
                horarios.append(
                    {
                        "key": str(sessao.sessao.horarioid.fim),
                        "value": sessao.sessao.horarioid.fim,
                    }
                )
        horarios.pop()
    ...
    def get_origem(self, dia, horario):
        first_session = (
            Inscricaosessao.objects.filter(inscricao=self, sessao__dia=dia)
            .order_by("sessao__horarioid__inicio")
            .first()
        )
        origem = []
        if horario == time.strftime(first_session.sessao.horarioid.inicio, "%H:%M:%S"):
            ...
            inscricao_sessoes = Inscricaosessao.objects.filter(
                inscricao=self, sessao__dia=dia, sessao__horarioid__fim=horario
            ).order_by("sessao__horarioid__inicio")
            for local in inscricao_sessoes:
                origem.append(
                    {
                        "key": local.sessao.atividadeid.espacoid.id,
    ...
    def get_destino(self, dia, horario, origem):
        ...
        if horario == time.strftime(
            inscricao_sessoes.first().sessao.horarioid.inicio, "%H:%M:%S"
        ):
            for local in inscricao_sessoes:
                if time.strftime(
                    local.sessao.horarioid.inicio, "%H:%M:%S"
                ) == horario and (
                    origem == "Check in"
                    or origem.id != local.sessao.atividadeid.espacoid.id
                ):
                    destino.append(
                        {
                            "key": local.sessao.atividadeid.espacoid.id,
            ...
            inscricao_sessoes = Inscricaosessao.objects.filter(
                inscricao=self, sessao__dia=dia, sessao__horarioid__inicio__gte=horario
            ).order_by("sessao__horarioid__inicio")
            for local in inscricao_sessoes:
                if (
                    origem != "Check in"
                    and origem.id != local.sessao.atividadeid.espacoid.id
                ):
                    destino.append(
                        {
                            "key": local.sessao.atividadeid.espacoid.id,
        ...
        if len(destino) == 0:
            if (
                ...
                destino.append(
                    {
                        "key": inscricao_sessoes.last().sessao.atividadeid.espacoid.id,
    ...
    def get_tarefa_grupos(facul):
        ...
        for i in inscricao:
            ...
            inscricao_sessao = Inscricaosessao.objects.filter(
                sessao__atividadeid__professoruniversitarioutilizadorid__faculdade__id=facul,
                inscricao__id=i.id,
            )
