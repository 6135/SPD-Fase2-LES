...
def enviar_mail_confirmacao_inscricao(request, pk):
    ...
    subject = (
        f"Confirmação da Inscrição no Dia Aberto de {ano} da Universidade do Algarve."
    )
    ...
    recipient_list = [
        inscricao.responsavel_set.first().email,
    ]
    pdf = render_pdf(
        "inscricoes/pdf.html", context, f"dia_aberto_ualg_{ano}.pdf"
    ).content
    email = EmailMessage(
        subject,
        message,
        source,
        recipient_list,
        attachments=[(f"dia_aberto_ualg_{ano}.pdf", pdf, "application/pdf")],
    )
...
def nao_tem_permissoes(request, inscricao):
    ...
    user_check_var = user_check(
        request=request, user_profile=[Coordenador, Participante, Administrador]
    )
...
def update_context(context, step, wizard=None, inscricao=None):
    ...
    if step == "escola":
        ...
        context.update(
            {
                "escolas": json.dumps(
                    list(set(map(lambda x: x.nome, Escola.objects.all())))
                ),
        ...
        dia = (
            wizard.get_cleaned_data_for_step("escola")["dia"]
            if wizard
            else inscricao.dia
        )
        pratos_info = {}
        ...
        context.update(
            {
                ...
                "nalunos": wizard.get_cleaned_data_for_step("escola")["nalunos"]
                if wizard
                else inscricao.nalunos,
                "nresponsaveis": 1,
            }
        )
    elif step == "sessoes":
        context.update(
            {
                "campus": json.dumps(
                    list(
                        map(
                            lambda x: {"id": x.id, "nome": x.nome}, Campus.objects.all()
                        )
                    )
                ),
                "unidades_organicas": json.dumps(
                    list(
                        map(
                            lambda x: {"id": x.id, "nome": x.nome},
                            Unidadeorganica.objects.all(),
                        )
                    )
                ),
                ...
                "publicos_alvo": json.dumps(
                    list(map(lambda x: x[0], Atividade.publicosalvo))
                ),
                "nalunos": wizard.get_cleaned_data_for_step("escola")["nalunos"]
                if wizard
                else inscricao.nalunos,
                "dia": wizard.get_cleaned_data_for_step("escola")["dia"].strftime(
                    "%d/%m/%Y"
                )
                if wizard
                else inscricao.dia.strftime("%d/%m/%Y"),
            }
        )
    elif step == "submissao":
        context.clear()
...
def update_post(step, POST, wizard=None, inscricao=None):
    ...
    if step == "escola":
        try:
            ...
            dia = pytz.utc.localize(datetime.strptime(POST[f"{prefix}dia"], "%d/%m/%Y"))
            diaaberto = Diaaberto.objects.filter(
                datadiaabertoinicio__lte=dia.replace(hour=23),
                datadiaabertofim__gte=dia.replace(hour=0),
            ).first()
            if diaaberto:
                POST[f"{prefix}diaaberto"] = diaaberto.id
        except:
            pass
        POST[f"{prefix}individual"] = (
            wizard.get_cleaned_data_for_step("info")["individual"]
            if wizard
            else inscricao.individual
        )
    elif step == "almoco":
        POST[f"{prefix}nalunos"] = (
            wizard.get_cleaned_data_for_step("escola")["nalunos"]
            if wizard
            else inscricao.nalunos
        )
        ...
        POST[f"{prefix}dia"] = (
            wizard.get_cleaned_data_for_step("escola")["dia"].strftime("%d/%m/%Y")
            if wizard
            else inscricao.dia.strftime("%d/%m/%Y")
        )
...
def save_form(request, step, form, inscricao):
    ...
    if step == "transporte":
        ...
        inscricao.hora_chegada = (
            form.cleaned_data["hora_chegada"] if not outro else None
        )
        inscricao.local_chegada = (
            form.cleaned_data["local_chegada"] if not outro else None
        )
