...
def filter_tipo(queryset, name, value):
    if value == "tarefaAcompanhar":
        return queryset.filter(
            Exists(TarefaAcompanhar.objects.filter(tarefaid=OuterRef("id")))
        )
    elif value == "tarefaAuxiliar":
        return queryset.filter(
            Exists(TarefaAuxiliar.objects.filter(tarefaid=OuterRef("id")))
        )
    elif value == "tarefaOutra":
        return queryset.filter(
            Exists(TarefaOutra.objects.filter(tarefaid=OuterRef("id")))
        )
    return queryset
...
class ColaboradorAtividadeFilter(django_filters.FilterSet):
    ...
    campus_id = django_filters.NumberFilter(field_name="espacoid__edificio__campus__id")
