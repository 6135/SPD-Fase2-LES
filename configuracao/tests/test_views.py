...
from configuracao.tests.test_models import (
    create_campus,
    create_curso,
    create_dep,
    create_edificio,
...
def url_with_querystring(path, **kwargs):
    ...
    )  # for Python 3, use urllib.parse.urlencode instead
...
class TestViews(TestCase):
    def setUp(self):
        ...
        self.menu = create_menu(
            campus=self.campus, horario=self.lunchTime, diaaberto=self.diaaberto
        )
    ...
    def test_ver_dias(self):
        ...
        response = client.get(
            reverse("configuracao:diasAbertos"), follow=True
        )  # Not logged in goes to message
        ...
        self.assertContains(
            response, status_code=200, text=str(self.diaaberto.ano)
        )  # vemos se o dia aparece na lista
        self.assertContains(
            response, status_code=200, text=str(self.diaaberto.descricao)
        )
        self.assertContains(response, status_code=200, text="Criar novo Dia Aberto")
        self.assertContains(
            response, status_code=200, text="Inicio submissao Atividades:"
        )  # vemos se o dia aparece na lista
        ...
        response = client.get(
            path=url_with_querystring(
                path=reverse("configuracao:diasAbertos"), ano=datetime.now().year
            )
        )  # apenas um ano
        self.assertContains(
            response, status_code=200, text="Inicio submissao Atividades:", count=1
        )  # vemos se o dia aparece na lista

        # print(url_with_querystring(path=reverse('configuracao:diasAbertos'), diainicio=str(date(1970,1,1))))
        response = client.get(
            path=url_with_querystring(
                path=reverse("configuracao:diasAbertos"),
                diainicio=str(date(1970, 1, 1)),
            )
        )  # agora as datas
        self.assertContains(
            response, status_code=200, text="Inicio submissao Atividades:", count=1
        ...
        response = client.get(
            path=url_with_querystring(
                path=reverse("configuracao:diasAbertos"), diafim=str(date(2040, 1, 2))
            )
        )
        self.assertContains(
            response, status_code=200, text="Inicio submissao Atividades:", count=1
        ...
        response = client.get(
            path=url_with_querystring(
                path=reverse("configuracao:diasAbertos"), ano=(datetime.now().year + 1)
            )
        )  # apenas um ano
        self.assertContains(
            response, status_code=200, text="Inicio submissao Atividades:", count=0
        )  # vemos se o dia aparece na lista

        response = client.get(
            path=url_with_querystring(
                path=reverse("configuracao:diasAbertos"),
                diainicio=str(date(2299, 5, 27)),
            )
        )  # agora as datas
        self.assertContains(
            response, status_code=200, text="Inicio submissao Atividades:", count=0
        ...
        response = client.get(
            path=url_with_querystring(
                path=reverse("configuracao:diasAbertos"), diafim=str(date(2299, 5, 27))
            )
        )
        self.assertContains(
            response, status_code=200, text="Inicio submissao Atividades:", count=0
    ...
    def test_novo_dia(self):
        ...
        response = client.get(
            reverse("configuracao:novoDia"), follow=True
        )  # Not logged in goes to message
        self.assertEquals(response.status_code, 200)
        self.assertTemplateUsed(response, "utilizadores/login.html")
        ...
        self.assertTemplateNotUsed(
            response, "configuracao/diaAbertoForm.html"
        )  # it cant be right because current day is still open
    ...
    def test_new_day_POST(self):
        ...
        )  # 302 means it inserted a redirected

        self.assertEquals(len(Diaaberto.objects.filter()), 1)
        self.assertEquals(
            Diaaberto.objects.filter(ano="1970").first().escalasessoes, time(0, 31)
        )
        ...
        data[
            "datadiaabertoinicio"
        ] = "2040-01-02 1287:30"  # formato errado, deve falhar
        response = client.post(reverse("configuracao:novoDia"), data=data)
        self.assertContains(
            response, status_code=200, text="Introduza uma data"
        )  # back to same page with error
        self.assertEquals(len(Diaaberto.objects.filter()), 0)

    def test_del_day(self):
        client = self.client
        response = client.get(
            reverse("configuracao:eliminarDia", kwargs={"id": self.diaaberto.id})
        )
        self.assertEquals(response.status_code, 302)  # Redirect if user not logged in
    ...
    def test_view_menus(self):
        ...
        response = client.get(
            reverse("configuracao:verMenus"), follow=True
        )  # Not logged in goes to message
        self.assertEquals(response.status_code, 200)
        self.assertTemplateUsed(response, "utilizadores/login.html")
    ...
    def test_menus_POST(self):
        ...
        response = client.get(
            reverse("configuracao:novoMenu"), follow=True
        )  # Not logged in goes to message
        self.assertEquals(response.status_code, 200)
        self.assertTemplateUsed(response, "utilizadores/login.html")
        ...
        response = client.post(
            reverse("configuracao:editarMenu", kwargs={"id": menuId}), data=data
        )
    ...
    def test_menu_delete(self):
        client = self.client
        response = client.get(
            reverse("configuracao:eliminarMenu", kwargs={"id": self.menu.id})
        )
        self.assertEquals(response.status_code, 302)  # Redirect if user not logged in
    ...
    def test_view_temas(self):
        ...
        response = client.get(
            reverse("configuracao:verTemas"), follow=True
        )  # Not logged in goes to message
        self.assertEquals(response.status_code, 200)
        self.assertTemplateUsed(response, "utilizadores/login.html")
    ...
    def test_temas_POST(self):
        ...
        response = client.get(
            reverse("configuracao:adicionarTema"), follow=True
        )  # Not logged in goes to message
        self.assertEquals(response.status_code, 200)
        self.assertTemplateUsed(response, "utilizadores/login.html")
        ...
        response = client.post(
            reverse("configuracao:editarTema", kwargs={"id": temaId}), data=data
        )
    ...
    def test_tema_delete(self):
        client = self.client
        response = client.get(
            reverse("configuracao:eliminarTema", kwargs={"id": self.tema.id})
        )
        self.assertEquals(response.status_code, 302)  # Redirect if user not logged in
    ...
    def test_view_uos(self):
        ...
        response = client.get(
            reverse("configuracao:verUOs"), follow=True
        )  # Not logged in goes to message
        self.assertEquals(response.status_code, 200)
        self.assertTemplateUsed(response, "utilizadores/login.html")
        ...
        response = client.get(reverse("configuracao:verUOs"))
        self.assertContains(
            response, status_code=200, text="Faculdade de Ciencias e Tecnologias"
        )
    ...
    def test_uos_POST(self):
        ...
        response = client.get(
            reverse("configuracao:adicionarUO"), follow=True
        )  # Not logged in goes to message
        self.assertEquals(response.status_code, 200)
        self.assertTemplateUsed(response, "utilizadores/login.html")
        ...
        self.assertEquals(
            Unidadeorganica.objects.all().first().nome, "Faculdade de TESTE"
        )
        ...
        response = client.post(
            reverse("configuracao:editarUO", kwargs={"id": uoId}), data=data
        )
    ...
    def test_uo_delete(self):
        client = self.client
        response = client.get(
            reverse("configuracao:eliminarUO", kwargs={"id": self.uo.id})
        )
        self.assertEquals(response.status_code, 302)  # Redirect if user not logged in

        response = client.get(
            reverse("configuracao:eliminarUO", kwargs={"id": self.uo.id}), follow=True
        )  # Not logged in goes to message
        self.assertEquals(response.status_code, 200)
        self.assertTemplateUsed(response, "utilizadores/login.html")
    ...
    def test_view_deps(self):
        ...
        response = client.get(
            reverse("configuracao:verDepartamentos"), follow=True
        )  # Not logged in goes to message
        self.assertEquals(response.status_code, 200)
        self.assertTemplateUsed(response, "utilizadores/login.html")
        ...
        self.assertContains(
            response,
            status_code=200,
            text="Departamento de Engenharia Informatica e Eletronica",
        )
    ...
    def test_deps_POST(self):
        ...
        response = client.get(
            reverse("configuracao:adicionarDepartamento"), follow=True
        )  # Not logged in goes to message
        self.assertEquals(response.status_code, 200)
        self.assertTemplateUsed(response, "utilizadores/login.html")
        ...
        response = client.post(
            reverse("configuracao:editarDepartamento", kwargs={"id": depId}), data=data
        )
        self.assertEquals(
            Departamento.objects.filter().first().nome,
    ...
    def test_dep_delete(self):
        client = self.client
        response = client.get(
            reverse("configuracao:eliminarDepartamento", kwargs={"id": self.dep.id})
        )
        self.assertEquals(response.status_code, 302)  # Redirect if user not logged in
    ...
    def test_view_edificios(self):
        ...
        response = client.get(
            reverse("configuracao:verEdificios"), follow=True
        )  # Not logged in goes to message
        self.assertEquals(response.status_code, 200)
        self.assertTemplateUsed(response, "utilizadores/login.html")
    ...
    def test_edificios_POST(self):
        ...
        response = client.get(
            reverse("configuracao:adicionarEdificio"), follow=True
        )  # Not logged in goes to message
        self.assertEquals(response.status_code, 200)
        self.assertTemplateUsed(response, "utilizadores/login.html")
        ...
        data["form-0-nome"] = ["2"]
        response = client.post(
            reverse("configuracao:editarEdificio", kwargs={"id": edificioId}), data=data
        )
    ...
    def test_edificio_delete(self):
        client = self.client
        response = client.get(
            reverse("configuracao:eliminarEdificio", kwargs={"id": self.edificio.id})
        )
        self.assertEquals(response.status_code, 302)  # Redirect if user not logged in
    ...
    def test_view_curso(self):
        ...
        response = client.get(
            reverse("configuracao:verCursos"), follow=True
        )  # Not logged in goes to message
        self.assertEquals(response.status_code, 200)
        self.assertTemplateUsed(response, "utilizadores/login.html")
        ...
        response = client.get(reverse("configuracao:verCursos"))
        self.assertContains(
            response, status_code=200, text="Licensiatura Engenharia Informatica"
        )
    ...
    def test_curso_POST(self):
        ...
        response = client.get(
            reverse("configuracao:adicionarCurso"), follow=True
        )  # Not logged in goes to message
        self.assertEquals(response.status_code, 200)
        self.assertTemplateUsed(response, "utilizadores/login.html")
        ...
        self.assertEquals(
            Curso.objects.all().first().nome, "Licensiatura Engenharia Informatica"
        )
        ...
        response = client.post(
            reverse("configuracao:editarCurso", kwargs={"id": cursoId}), data=data
        )
        self.assertEquals(
            Curso.objects.get(id=cursoId).nome, "Licensiatura Engenharia Eletronica"
        )
        Curso.objects.all().delete()
    ...
    def test_curso_delete(self):
        client = self.client
        response = client.get(
            reverse("configuracao:eliminarCurso", kwargs={"id": self.curso.id})
        )
        self.assertEquals(response.status_code, 302)  # Redirect if user not logged in
