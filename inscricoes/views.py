...
from inscricoes.utils import (
    add_vagas_sessao,
    enviar_mail_confirmacao_inscricao,
    init_form,
    nao_tem_permissoes,
    render_pdf,
    save_form,
...
class CriarInscricao(SessionWizardView):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if _user_check["exists"]:
            ...
            if (
                datetime.now(pytz.UTC) < diaaberto.datainscricaoatividadesinicio
                or datetime.now(pytz.UTC) > diaaberto.datainscricaoatividadesfim
            ):
            ...
            self.instance_dict = {
                "responsaveis": Responsavel(
                    nome=f"{participante.first_name} {participante.last_name}",
                    email=participante.email,
                    tel=participante.contacto,
                )
            }
        else:
    ...
    def get_context_data(self, form, **kwargs):
        ...
        context.update({"visited": visited})
        return context
    ...
    def done(self, form_list, form_dict, **kwargs):
        ...
        inscricao.participante = Participante.objects.filter(
            utilizador_ptr_id=self.request.user.id
        ).first()
        inscricao.meio_transporte = form_dict["transporte"].cleaned_data["meio"]
        if form_dict["transporte"].cleaned_data["meio"] != "outro":
            inscricao.hora_chegada = form_dict["transporte"].cleaned_data[
                "hora_chegada"
            ]
            inscricao.local_chegada = form_dict["transporte"].cleaned_data[
                "local_chegada"
            ]
        ...
        for sessaoid in sessoes:
            if sessoes[sessaoid] > 0:
                inscricao_sessao = Inscricaosessao(
                    sessao=Sessao.objects.get(pk=sessaoid),
                    nparticipantes=sessoes[sessaoid],
                    inscricao=inscricao,
                )
        ...
        return render(
            self.request,
            "inscricoes/consultar_inscricao_submissao.html",
            {
                "inscricao": inscricao,
            },
        )
...
class ConsultarInscricao(View):
    ...
    def get(self, request, pk, step=0, alterar=False):
        ...
        if erro_permissoes:
            return erro_permissoes
        if (
            user_check(request, [Participante])["exists"]
            and datetime.now(pytz.UTC) > inscricao.diaaberto.datainscricaoatividadesfim
        ):
        ...
        return render(
            request, f"{self.template_prefix}_{self.step_names[step]}.html", context
        )
    ...
    def post(self, request, pk, step=0, alterar=False):
        ...
        if alterar:
            if (
                request.user.groups.filter(name="Participante").exists()
                and datetime.now(pytz.UTC)
                > inscricao.diaaberto.datainscricaoatividadesfim
            ):
                m = f"Não pode alterar a inscrição fora do período: {inscricao.diaaberto.datainscricaoatividadesinicio.strftime('%d/%m/%Y')} até {inscricao.diaaberto.datainscricaoatividadesfim.strftime('%d/%m/%Y')}"
                return render(
                    request=request,
            ...
            if self.step_names[step] == "sessoes":
                for inscricao_sessao in inscricoessessao:
                    add_vagas_sessao(
                        inscricao_sessao.sessao.id, inscricao_sessao.nparticipantes
                    )
            if form.is_valid():
                save_form(request, self.step_names[step], form, inscricao)
                return HttpResponseRedirect(
                    reverse(
                        "inscricoes:consultar-inscricao",
                        kwargs={"pk": pk, "step": step},
                    )
                )
            if self.step_names[step] == "sessoes":
                for inscricao_sessao in inscricoessessao:
...
class ConsultarInscricoes(SingleTableMixin, FilterView):
    ...
    def get_filterset_kwargs(self, filterset_class):
        kwargs = super(ConsultarInscricoes, self).get_filterset_kwargs(filterset_class)
        if kwargs["data"] is None:
            kwargs["data"] = {
                "diaaberto": Diaaberto.objects.filter(ano__lte=datetime.now().year)
                .order_by("-ano")
                .first()
                .id
            }
            ...
            kwargs["data"]["diaaberto"] = (
                Diaaberto.objects.filter(ano__lte=datetime.now().year)
                .order_by("-ano")
                .first()
                .id
            )
...
class InscricoesUO(ConsultarInscricoes):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        self.queryset = Inscricao.objects.filter(
            Exists(
                Inscricaosessao.objects.filter(
    ...
    def get_context_data(self, **kwargs):
        ...
        context["departamentos"] = list(
            map(
                ...
                Departamento.objects.filter(unidadeorganicaid=coordenador.faculdade),
            )
        )
        return context
...
def estatisticas(request, diaabertoid=None):
    ...
    dias = [
        (diaaberto.datadiaabertoinicio + timedelta(days=x)).strftime("%d/%m/%Y")
        for x in range(numdays)
    ]
    return render(
        request,
        "inscricoes/estatisticas.html",
        {
            "diaaberto": diaaberto,
