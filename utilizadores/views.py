    ...
    if not request.user.is_authenticated:
        ...
        for profile in user_profile:
            if profile.objects.filter(utilizador_ptr_id=request.user.id).exists():
                return {
                    "exists": True,
                    "firstProfile": profile.objects.filter(
...
def load_departamentos(request):
    ...
    departamentos = Departamento.objects.filter(unidadeorganicaid=faculdadeid).order_by(
        "nome"
    )
    return render(
        request,
        "utilizadores/departamento_dropdown_list_options.html",
...
def load_cursos(request):
    ...
    return render(
        request, "utilizadores/curso_dropdown_list_options.html", {"cursos": cursos}
    )
...
def escolher_perfil(request):
    ...
    return render(
        request=request,
        template_name="utilizadores/escolher_perfil.html",
        ...
        if form.is_valid():
            ...
            if tipo == 1:
                ...
                views.enviar_notificacao_automatica(
                    request, "validarRegistosPendentes", recipient_id
...
def validar_utilizador(request, id):
    ...
    try:
        ...
        message = "Caro(a) " + u.first_name + "\n\n"
        message += (
            "O seu registo na plataforma do dia aberto foi bem sucedido!" + ",\n\n"
        )
        message += "Equipa do dia aberto da Ualg"
        email_from = settings.EMAIL_HOST_USER
        recipient_list = [
...
def apagar_utilizador(request, id):
    ...
    mensagens_recebidas1 = MensagemRecebida.objects.select_related(
        "mensagem__recetor"
    ).filter(mensagem__recetor=utilizador.id)
    for msg in mensagens_recebidas1:
        msg.delete()
    mensagens_recebidas2 = MensagemRecebida.objects.select_related(
        "mensagem__emissor"
    ).filter(mensagem__emissor=utilizador.id)
    for msg in mensagens_recebidas2:
        msg.delete()
    mensagens_enviadas1 = MensagemEnviada.objects.select_related(
        "mensagem__recetor"
    ).filter(mensagem__recetor=utilizador.id)
    for msg in mensagens_enviadas1:
        msg.delete()
    mensagens_enviadas2 = MensagemEnviada.objects.select_related(
        "mensagem__emissor"
    ).filter(mensagem__emissor=utilizador.id)
...
def alterar_utilizador_admin(request, id):
    ...
    if user.groups.filter(name="Coordenador").exists():
        ...
        utilizador_form = ProfessorUniversitarioAlterarPerfilForm(
            instance=utilizador_object
        )
        perfil = "Professor Universitario"
    elif user.groups.filter(name="Colaborador").exists():
        tipo = 4
    ...
    if request.method == "POST":
        ...
        if tipo == 1:
            form = ParticipanteAlterarPerfilForm(
                submitted_data, instance=utilizador_object
            )
            my_group = Group.objects.get(name="Participante")
        elif tipo == 2:
        ...
        if (
            email
            and User.objects.exclude(email=utilizador_object.email)
            .filter(email=email)
            .exists()
        ):
...
def alterar_utilizador(request):
    ...
    if request.method == "POST":
        ...
        if (
            username
            and User.objects.exclude(username=utilizador_object.username)
...
def mudar_perfil_escolha_admin(request, id):
    ...
    return render(
        request=request,
        template_name="utilizadores/mudar_perfil_escolha_admin.html",
        context={"utilizadores": utilizadores, "u": u, "id": id, "x": x},
    )
...
def mudar_perfil_escolha(request):
    ...
    return render(
        request=request,
        template_name="utilizadores/mudar_perfil_escolha.html",
        context={"utilizadores": utilizadores, "u": u, "id": id, "x": x},
    )
...
def mudar_perfil(request, tipo):
    ...
    if request.method == "POST":
        ...
        if form.is_valid() and len(erros) == 0:
            ...
            if (
                tipo == 2 or tipo == 3 or tipo == 4 or tipo == 5
            ):  # Enviar Notificacao Automatica !!!!!!!!!
                recipient_id = (
                    utilizador_form_object.id
                )  # Enviar Notificacao Automatica !!!!!!!!!
                views.enviar_notificacao_automatica(
                    request, "validarAlteracoesPerfil", recipient_id
                )  # Enviar Notificacao Automatica !!!!!!!!!
            return redirect("utilizadores:logout")
        else:
            msg = True
