...
def get_due_date_string(value, id):
    ...
    passou = tarefa.horario.strftime("%H:%M:%S") < datetime.datetime.now().strftime(
        "%H:%M:%S"
    )
    value = tarefa.dia
    ...
    if delta.days == 0 and tarefa.estado != "Concluida":
        ...
        return "Esta tarefa foi há %s %s atrás!" % (
            abs(delta.days),
...
def tarefa_passou(value, id):
    ...
    if delta.days > 0:
        ...
        return tarefa.horario.strftime("%H:%M:%S") < datetime.datetime.now().strftime(
            "%H:%M:%S"
        )
...
def iniciar_tarefa(value, id):
    tarefa = Tarefa.objects.get(id=id)
    value = tarefa.dia
    comecou = tarefa.horario.strftime("%H:%M:%S") < datetime.datetime.now().strftime(
        "%H:%M:%S"
    )
    delta = value - date.today()
...
def get_tarefa_auxiliar_prof(tarefa, id):
    ...
    if tarefa != None:
        return (
            tarefa.sessao.atividadeid.professoruniversitarioutilizadorid.first_name
            + " "
            + tarefa.sessao.atividadeid.professoruniversitarioutilizadorid.last_name
        )
    else:
        pass
