...
def create_Edificio_1():
    return Edificio.objects.get_or_create(
        nome="C2", campus=create_Campus_1(), image="images/edifi/gambelas.jpg"
    )[0]
...
def create_Horario_0():
    return Horario.objects.get_or_create(inicio=time(12, 0), fim=time(14, 30))[0]
...
def create_Tarefa_Auxiliar():
    return TarefaAuxiliar.objects.get_or_create(
        tarefaid=create_Tarefa_0(), sessao=create_Sessao_0()
    )[0]
...
def create_Tarefa_Outra():
    return TarefaOutra.objects.get_or_create(
        tarefaid=create_Tarefa_2(), descricao="Fechar a sala 1.63 do Edificio 1"
    )[0]
