...
class AtividadesProfessor(SingleTableMixin, FilterView):
    ...
    def dispatch(self, request, *args, **kwargs):
        user_check_var = user_check(
            request=request, user_profile=[ProfessorUniversitario]
        )
    ...
    def get_queryset(self):
        return (
            Atividade.objects.filter(
                professoruniversitarioutilizadorid=self.user_check_var.get(
                    "firstProfile"
                )
            )
            .order_by("-id")
...
class AtividadesCoordenador(SingleTableMixin, FilterView):
    ...
    def get_queryset(self):
        return (
            Atividade.objects.filter(
                professoruniversitarioutilizadorid__faculdade=self.user_check_var.get(
                    "firstProfile"
                ).faculdade
            )
            .order_by("-id")
...
def conflict_array():
    ...
    for sessao1 in sessoes:
        for sessao2 in sessoes:
            if (
                sessao1.id != sessao2.id
                and sessao1.atividadeid != sessao2.atividadeid
                and sessao1.atividadeid.espacoid == sessao2.atividadeid.espacoid
                and sessao1.dia == sessao2.dia
            ):
                hora1inicio = (
                    sessao1.horarioid.inicio.hour * 60 + sessao1.horarioid.inicio.minute
                )
                hora1fim = (
                    sessao1.horarioid.fim.hour * 60 + sessao1.horarioid.fim.minute
                )
                hora2inicio = (
                    sessao2.horarioid.inicio.hour * 60 + sessao2.horarioid.inicio.minute
                )
                hora2fim = (
                    sessao2.horarioid.fim.hour * 60 + sessao2.horarioid.fim.minute
                )
                if (
                    hora1inicio <= hora2inicio < hora1fim
                    or hora1inicio < hora2fim <= hora1fim
                ):
...
def alterarAtividade(request, id):
    ...
    if atividade.exists():
        activity_object = Atividade.objects.get(
            id=id
        )  # Objecto da atividade que temos de mudar, ativdade da dupla
        if (
            activity_object.professoruniversitarioutilizadorid
            != ProfessorUniversitario.objects.get(utilizador_ptr_id=request.user.id)
        ):
        ...
        )  # Formulario instanciado pela atividade a mudar
        espaco = Espaco.objects.get(id=activity_object.espacoid.id)
        materiais_object = Materiais.objects.get(atividadeid=id)
        new_material = Materiais(
            atividadeid=activity_object, nomematerial=materiais_object
        )
        ...
        if (
            request.method == "POST"
        ):  # Se estivermos a receber um request com formulario
            submitted_data = request.POST.copy()
            activity_object.tema = Tema.objects.get(id=int(request.POST["tema"]))
            activity_object_form = AtividadeForm(
                submitted_data, instance=activity_object
            )
            materiais_object_form = MateriaisForm(
                request.POST, instance=materiais_object
            )
                ...
                if activity_object_formed.estado == "nsub":
                    ...
                    for sessao in sessoes:
                        ...
                        horario = Horario.objects.filter(
                            inicio=sessao.horarioid.inicio, fim=hfim
                        ).first()
                        if horario is None:
                    ...
                    if Atividade.objects.get(id=id).ne(
                        activity_object_formed
                    ) or Materiais.objects.get(atividadeid=id).ne(
                        materiais_object_form.instance
                    ):
                        espacoid = request.POST["espacoid"]
                        espaco = Espaco.objects.get(id=espacoid)
                        activity_object_formed.espacoid = espaco
                        ...
                        sessoes = Sessao.objects.filter(
                            atividadeid=activity_object_formed
                        )
                        print(sessoes)
                        for sessao in sessoes:
                            inicio = str(sessao.horarioid.inicio)
...
def proporatividade(request):
    ...
    diaabertopropostas = Diaaberto.objects.get(
        datapropostasatividadesincio__lte=today, dataporpostaatividadesfim__gte=today
    )
    ...
    if request.method == "POST":
        ...
        espaco = Espaco.objects.get(id=espacoid)
        new_form = Atividade(
            professoruniversitarioutilizadorid=ProfessorUniversitario.objects.get(
    ...
    return render(
        request,
        "atividades/proporAtividadeAtividade.html",
        {
            "form": activity_object_form,
    ...
    if atividade.exists():
        ...
        if request.method == "POST":
            if "new" in request.POST:
                ...
                hfim = horariofim(splitinicio, duracaoesperada)
                horario = Horario.objects.filter(
                    inicio=request.POST["horarioid"], fim=hfim
                ).first()
                if horario is None:
                    new_Horario = Horario(inicio=inicio, fim=hfim)
                    new_Horario.save()
...
def veredificios(request):
    ...
    return render(
        request,
        template_name="atividades/generic_list_options.html",
...
def verhorarios(request):
    ...
    default = {"key": "", "value": "Escolha um horario"}
    ...
    if id != -1:
        ...
        horar2 = []
        horar3 = []
        escala = Diaaberto.objects.get(
            datapropostasatividadesincio__lte=today,
            dataporpostaatividadesfim__gte=today,
        ).escalasessoes.minute
        print(escala)
        if len(sessaodia) == 0:
            options = [
                {
                ...
                for session_time in Diaaberto.objects.get(
                    datapropostasatividadesincio__lte=today,
                    dataporpostaatividadesfim__gte=today,
                ).session_times()
            ]
            ...
            for sessao in sessaodia:
                timeinicio = TimeC(
                    time=str(sessao.horarioid.inicio.hour)
                    + ":"
                    + str(sessao.horarioid.inicio.minute)
                )
                timefim = TimeC(
                    time=str(sessao.horarioid.fim.hour)
                    + ":"
                    + str(sessao.horarioid.fim.minute)
                )
...
def validaratividade(request, id, action):
    ...
    if action == 0:
        nviews.enviar_notificacao_automatica(
            request, "rejeitarAtividade", id
        )  # Enviar Notificacao Automatica !!!!!!
        atividade.estado = "Recusada"
    if action == 1:
        nviews.enviar_notificacao_automatica(
            request, "confirmarAtividade", id
        )  # Enviar Notificacao Automatica !!!!!!
...
def confirmarResumo(request, id):
    ...
    if atividade.exists():
        ...
        if atividade.estado == "nsub":
            ...
            nviews.enviar_notificacao_automatica(
                request, "atividadeAlterada", atividade.id
            )  # Enviar Notificacao Automatica !!!!!!
        return redirect("atividades:minhasAtividades")
    else:
        return render(
...
def verfaculdades(request):
    ...
    if (
        value_campus != "None"
        and value_campus is not None
        and value_campus is not False
    ):
    ...
    if value_uo != "None" and value_uo is not None and value_uo is not False:
        ...
        default = {"key": uo.id, "value": uo.nome}
    else:
        default = {"key": "", "value": "Qualquer Faculdade"}
    return render(
        request=request,
        template_name="configuracao/dropdown.html",
