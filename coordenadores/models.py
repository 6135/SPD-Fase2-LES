...
class Tarefa(models.Model):
    ...
    coord = models.ForeignKey(
        "utilizadores.Coordenador",
        models.CASCADE,
        db_column="CoordenadorUtilizadorID",
        null=True,
    )  # Field name made lowercase.
    colab = models.ForeignKey(
        "utilizadores.Colaborador",
        models.SET_NULL,
        db_column="ColaboradorUtilizadorID",
        null=True,
        blank=True,
    )  # Field name made lowercase.
    ...
    diaaberto = models.ForeignKey(
        "configuracao.Diaaberto", models.CASCADE, db_column="Diaaberto"
    )  # Field name made lowercase.
    ...
    def eliminar(self):
        if (
            (self.estado == "Iniciada" and self.dia < date.today())
            or self.estado == "naoConcluida"
            or self.estado == "Concluida"
            or self.estado == "Cancelada"
            or self.estado == "naoAtribuida"
        ):
            return True
        return False
...
class TarefaAcompanhar(models.Model):
    tarefaid = models.OneToOneField(
        Tarefa, models.CASCADE, db_column="tarefaid", primary_key=True
    )
    origem = models.CharField(
        max_length=255, db_column="origem", blank=False, null=False
    )
    destino = models.CharField(
        max_length=255, db_column="destino", blank=False, null=False
    )
    inscricao = models.ForeignKey(
        "inscricoes.Inscricao", models.CASCADE, db_column="inscricao"
    )
    ...
    def getDescription(self):
        ...
        msg = (
            ...
            + destino.nome
            + ", no edifício "
            + destino.edificio.nome
            + ", no dia "
            + self.tarefaid.dia.strftime("%d/%m/%y")
            + " às "
            + self.tarefaid.horario.strftime("%H horas e %M minutos")
            + "."
        )
        return msg
...
def remove_parent(sender, instance, **kwargs):
    ...
    sender.objects.raw(
        "DELETE FROM " + str(sender.__name__) + " WHERE tarefaid =" + str(tarefaid.id)
    )
