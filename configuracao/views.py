...
from configuracao.tables import (
    CursoTable,
    DepartamentoTable,
    DiaAbertoTable,
    EdificioTable,
...
from configuracao.filters import (
    CursoFilter,
    DepartamentoFilter,
    DiaAbertoFilter,
    EdificioFilter,
...
class TimeC:
    ...
    def __init__(self, time: str = None, time_as_seconds: int = None):
        ...
        if time is not None:
            ...
            self.seconds = (
                int(self.time_split[0]) * 60 * 60 + int(self.time_split[1]) * 60
            )
            self.__str__()
        else:
            self.time = (
                str(int(time_as_seconds / 60 / 60))
                + ":"
                + str(int(time_as_seconds % 3600))
            )
...
class viewDays(SingleTableMixin, FilterView):
    ...
    def get_context_data(self, **kwargs):
        ...
        earliest = (
            Diaaberto.objects.all().order_by("ano").first()
...
def menuPratoFormset(extra=0, minVal=1, max=1000):
    formSets = modelformset_factory(
        model=Prato,
        exclude=["id", "menuid"],
        widgets={
            "tipo": Select(attrs={"class": "input"}),
            "prato": TextInput(attrs={"class": "input"}),
            "nrpratosdisponiveis": NumberInput(
                attrs={"class": "input", "min": "1", "style": "width: 30%"}
            ),
        },
        labels={"nrpratosdisponiveis": "# Pratos"},
        extra=extra,
        min_num=minVal,
        max_num=max,
        can_delete=True,
...
def newPratoRow(request):
    ...
    return render(
        request=request, template_name="configuracao/menuPratoRow.html", context=data
    )
        ...
        if "default" in request.POST and request.POST["default"] != "None":
            ...
            if request.POST["typeForm"] == "transporte":
                default = {
                    ...
                    "value": str(
                        Transporte.objects.get(id=request.POST["default"]).dia
                    ),
                }
...
def criarTransporte(request, id=None):
    ...
    if Diaaberto.current() is None:
        return {
            ...
            "render": render(
                ...
                context={"tipo": "error", "m": "Crie um dia aberto primeiro!"},
            ),
        }
    # vars
    transport_by_default = Transporte()
    transport_universitario_default = Transporteuniversitario(
        transporte=transport_by_default
    )
    # forms
    ...
    if id is not None:
        ...
        horario_form_set = HorarioFormSet(
            queryset=Transportehorario.objects.filter(transporte=transport_by_default)
        )
        form_transport = transporteForm(instance=transport_by_default)
        form_universitario = transporteUniversitarioForm(
            instance=Transporteuniversitario.objects.get(
                transporte=transport_by_default
            )
        )
    ...
    if request.method == "POST":
        ...
        form_universitario = transporteUniversitarioForm(
            request.POST, instance=transport_universitario_default
        )
        horario_form_set = HorarioFormSet(request.POST)
        if (
            form_transport.is_valid()
            and form_universitario.is_valid()
            and horario_form_set.is_valid()
        ):
...
def newHorarioRow(request):
    ...
    return render(
        request=request,
        template_name="configuracao/transporteHorarioEmptyRow.html",
        context=data,
    )
...
def atribuirTransporte(request, id):
    ...
    inscricaotransporte = Inscricaotransporte.objects.filter(
        transporte=transportehorario.id
    )
    ocupadas = 0
    ...
    # print(ocupadas)
    transportevagas = (
        transportehorario.transporte.transporteuniversitario.capacidade - ocupadas
    )
    inscricoestotais = Inscricao.objects.filter(
        nalunos__lte=transportevagas, dia=transportehorario.transporte.dia
    ).exclude(hora_chegada=None)
    inscricoes = []
    chepart = 0
    dadoschepart = []
    ...
    for inscricao in inscricoes:
        isessaochegada = (
            Inscricaosessao.objects.filter(inscricao=inscricao)
            .order_by("sessao__horarioid__inicio")
            .first()
        )
        if isessaochegada != None:
            if isessaochegada.sessao.dia == transportehorario.transporte.dia:
                if (
                    transportehorario.origem == "Gambelas"
                    or transportehorario.origem == "Penha"
                ):
                    isessaopartida = (
                        Inscricaosessao.objects.filter(inscricao=inscricao.id)
                        .order_by("-sessao__horarioid__inicio")
                        .first()
                    )  # ultima sessao da inscricao
                    isessaopartidalocal = (
                        isessaopartida.sessao.atividadeid.espacoid.edificio.campus.nome
                    )  # campus ultima sessao da inscricao
                    isessaopartidahorario = (
                        isessaopartida.sessao.horarioid.fim
                    )  # horario de fim da ultima sessao
                    horapartida = (
                        transportehorario.horaPartida.hour * 60
                        + transportehorario.horaPartida.minute
                    ) - (
                        isessaopartidahorario.hour * 60 + isessaopartidahorario.minute
                    )  # diferença entre horario transporte e da ultima sessao
                    if (
                        isessaopartidalocal == transportehorario.origem
                        and horapartida <= 60
                        and horapartida >= 0
                    ):
                        chepart = ChegadaPartida(
                            inscricao.id,
                            inscricao.nalunos,
                    ...
                    isessaochegadalocal = (
                        isessaochegada.sessao.atividadeid.espacoid.edificio.campus.nome
                    )
                    horachegada = (
                        transportehorario.horaChegada.hour * 60
                        + transportehorario.horaChegada.minute
                    ) - (
                        inscricao.hora_chegada.hour * 60 + inscricao.hora_chegada.minute
                    )
                    if (
                        isessaochegadalocal == transportehorario.chegada
                        and horachegada <= 60
                        and horachegada >= 0
                    ):
                        chepart = ChegadaPartida(
    ...
    if request.method == "POST":
        ...
        if "new" in request.POST:
            ...
            new_inscricaotransporte = Inscricaotransporte(
                transporte=transportehorario,
                npassageiros=grupo.nalunos,
                inscricao=grupo,
            )
    ...
    return render(
        ...
        context={
            "transporte": transportehorario,
            "inscricoestransporte": inscricaotransporte,
            "vagas": transportevagas,
            "chegadapartida": dadoschepart,
        },
    )
...
def configurarEdificio(request, id=None):
    ...
    espacoFormSet = modelformset_factory(
        model=Espaco, form=EspacoForm, extra=0, min_num=1, can_delete=True
    )
    ...
    return render(
        ...
        context={"form": edificioForm, "formset": formSet},
    )
...
def newEspacoRow(request):
    ...
    return render(
        request=request,
        template_name="configuracao/edificioEspacoRow.html",
        context=data,
    )
...
def newUORow(request):
    ...
    return render(
        request=request, template_name="configuracao/UORow.html", context=data
    )
...
class verDepartamentos(SingleTableMixin, FilterView):
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context["facs"] = list(
            map(lambda x: (x.id, x.nome), Unidadeorganica.objects.all())
        )
        return context
...
def configurarDepartamento(request, id=None):
    ...
    if id is not None:
        ...
        departamentoforms = departamentoformSet(
            queryset=Departamento.objects.filter(id=departamento.id)
        )
        allowMore, allowDelete = False, False
...
def newDepartamentoRow(request):
    ...
    return render(
        request=request, template_name="configuracao/departamentoRow.html", context=data
    )
...
def configurarCurso(request, id=None):
    ...
    if id is not None:
        ...
        departamentoforms = departamentoformSet(
            queryset=Curso.objects.filter(id=departamento.id)
        )
        allowMore, allowDelete = False, False
