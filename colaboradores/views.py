...
class AtividadesColaboradorSelecionadas(SingleTableMixin, FilterView):
    ...
    def get_queryset(self):
        return self.user_check_var.get(
            "firstProfile"
        ).get_atividades_escolhidas_tabela()
...
def minha_disponibilidade(request):
    ...
    preferencia_auxiliar = Preferencia.objects.filter(
        colab=u, tipoTarefa="tarefaAuxiliar"
    )
    ...
    preferencia_acompanhar = Preferencia.objects.filter(
        colab=u, tipoTarefa="tarefaAcompanhar"
    )
    ...
    if request.method == "POST":
        ...
        if values.is_valid() and horario_form_set.is_valid():
            ...
            if "tarefaAcompanhar" in tarefas:
                preferencia = Preferencia.objects.get_or_create(
                    colab=u, tipoTarefa="tarefaAcompanhar"
                )[0]
            else:
                preferencia_acompanhar = Preferencia.objects.filter(
            ...
            if "tarefaOutra" in tarefas:
                preferencia = Preferencia.objects.get_or_create(
                    colab=u, tipoTarefa="tarefaOutra"
                )[0]
            else:
                preferencia_outra = Preferencia.objects.filter(
                    colab=u, tipoTarefa="tarefaOutra"
                )
                if len(preferencia_outra) > 0:
    ...
    return render(
        ...
        context={
            "form_preferencia_tarefas": form_preferencia_tarefas,
            "horario_form_set": horario_form_set,
            "dias": Diaaberto.current().days_as_array(),
            "msg": msg,
            "erros": erros,
            "tarefa_auxiliar": tarefa_auxiliar,
            "tarefa_acompanhar": tarefa_acompanhar,
            "tarefa_outra": tarefa_outra,
        },
    )
...
def newHorarioRow(request):
    ...
    return render(
        request=request,
        template_name="colaboradores/preferencia_horario.html",
        context=data,
    )
...
def concluir_disponibilidade(request):
    ...
    return render(
        request=request, template_name="colaboradores/concluir_disponibilidade.html"
    )
...
def selecionar_atividade(request, id):
    ...
    if request.user.is_authenticated:
        ...
        return redirect("utilizadores:mensagem", 5)
    msg = True
    preferencia_auxiliar = Preferencia.objects.get_or_create(
        colab=u, tipoTarefa="tarefaAuxiliar"
    )[0]
    atividade = Atividade.objects.get(id=id)
    preferencia_atividade = PreferenciaAtividade(
        preferencia=preferencia_auxiliar, atividade=atividade
    )
...
def retirar_atividade(request, id):
    ...
    atividade = Atividade.objects.get(id=id)
    preferencia_atividade = PreferenciaAtividade.objects.get_or_create(
        preferencia=preferencia_auxiliar, atividade=atividade
    )[0]
...
def preferencia_atividade(request):
    ...
    if request.user.is_authenticated:
        ...
        return redirect("utilizadores:mensagem", 5)
    return render(
        request=request, template_name="colaboradores/preferencia_atividade.html"
    )
...
def validar_cancelamento_tarefa(request, id_notificacao):
    ...
    try:
        ...
        tarefa.save()
        views.enviar_notificacao_automatica(
            request, "confirmarCancelarTarefa", id_tarefa
        )
    except:
...
def rejeitar_cancelamento_tarefa(request, id_notificacao):
    ...
    try:
        ...
        id_tarefa = notificacao.action_object.id
        views.enviar_notificacao_automatica(
            request, "rejeitarCancelarTarefa", id_tarefa
        )
    except:
        return redirect("utilizadores:mensagem", 11)
    return HttpResponseRedirect(request.META.get("HTTP_REFERER", "/"))
