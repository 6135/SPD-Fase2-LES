...
class Transporte(models.Model):
    ...
    diaaberto = models.ForeignKey("Diaaberto", models.CASCADE, db_column="diaAberto")
...
class Transportehorario(models.Model):
    ...
    origem = models.CharField(
        db_column="Origem", max_length=32, blank=True, null=False, choices=choices
    )
    # Field name made lowercase.
    chegada = models.CharField(
        db_column="Chegada", max_length=32, blank=True, null=False, choices=choices
    )
    # Field name made lowercase.
    ...
    transporte = models.ForeignKey(Transporte, models.CASCADE, db_column="Transporte")
    ...
    def __str__(self):
        return (
            ...
            + self.chegada
            + " Horas: "
            + str(self.horaChegada)
            + " - "
            + str(self.horaPartida)
            + " "
            + str(self.transporte)
        )
    ...
    def get_capacidade(self):
        return Transporteuniversitario.objects.get(
    ...
    def get_trip_time(self):
        return (
            datetime_time.strftime(self.horaPartida, "%H:%M")
            + " - "
            + datetime_time.strftime(self.horaChegada, "%H:%M")
        )
...
class Diaaberto(models.Model):
    ...
    administradorutilizadorid = models.ForeignKey(
        ...
        models.SET_NULL,
        db_column="AdministradorUtilizadorID",
        null=True,
    )
    # Field name made lowercase.
        ...
        return [
            (
                time.strftime(
                    "%H:%M",
                    time.gmtime(
                        start_time_as_seconds + (n * self.escalasessoes.minute * 60)
                    ),
                )
            )
            for n in range(
                int((43200 - start_time_as_seconds) / (self.escalasessoes.minute * 60))
            )
        ] + [
            (
                time.strftime(
                    "%H:%M",
                    time.gmtime(time_lunch_end + (n * self.escalasessoes.minute * 60)),
                )
            )
            for n in range(
                int(
                    (end_time_as_seconds - time_lunch_end)
                    / (self.escalasessoes.minute * 60)
                )
                + 1
            )
        ]
    ...
    def singup_open():
        ...
        if current is not None:
            return (
                current.datainscricaoatividadesinicio <= now
                and current.datainscricaoatividadesfim > now
            )
        else:
            return False
    ...
    def submit_activities_open():
        ...
        if current is not None:
            return (
                current.datapropostasatividadesincio <= now
                and current.dataporpostaatividadesfim > now
            )
        else:
            return False
    ...
    def days_as_array(self):
        ...
        total_dias = data_fim - data_inicio + timedelta(days=1)
        return [
            ((data_inicio + timedelta(days=d)).date()) for d in range(total_dias.days)
        ]
...
class Menu(models.Model):
    ...
    id = models.AutoField(db_column="ID", primary_key=True)
    # Field name made lowercase.
    horarioid = models.ForeignKey("Horario", models.CASCADE, db_column="HorarioID")
    campus = models.ForeignKey("Campus", models.CASCADE, db_column="Campus")
    diaaberto = models.ForeignKey("Diaaberto", models.CASCADE, db_column="diaAberto")
...
class Prato(models.Model):
    ...
    # Field name made lowercase.
    nrpratosdisponiveis = models.IntegerField(
        db_column="NrPratosDisponiveis", blank=False, null=False
    )
...
class Espaco(models.Model):
    ...
    edificio = models.ForeignKey(
        "Edificio", models.CASCADE, db_column="Edificio", blank=True, null=False
    )  # Field name made lowercase.
    andar = models.CharField(
        db_column="Andar", max_length=255, blank=True, null=False
    )  # Field name made lowercase.
    ...
    def __str__(self):
        if self.edificio.image:
            return mark_safe(
                "<a href='"
                + reverse(
                    "configuracao:verEdificioImagem", kwargs={"id": self.edificio.id}
                )
                + "'>"
                + escape(self.nome)
                + "</a>"
...
class Edificio(models.Model):
    ...
    nome = models.CharField(
        db_column="Nome", max_length=64
    )  # Field name made lowercase.
    campus = models.ForeignKey(
        "Campus", models.CASCADE, db_column="Campus", null=False
    )  # Field name made lowercase.
    image = models.ImageField(
        upload_to="images/edifi", db_column="image", blank=True, null=True
    )
    ...
    def __str__(self):
        if self.image:
            return mark_safe(
                "<a href='"
                + reverse("configuracao:verEdificioImagem", kwargs={"id": self.id})
                + "'>"
                + escape(self.nome)
                + "</a>"
            )
...
class Departamento(models.Model):
    ...
    # Field name made lowercase.
    sigla = models.CharField(db_column="Sigla", max_length=32, blank=True, null=True)
    # Field name made lowercase.
...
class Idioma(models.Model):
    ...
    id = models.AutoField(db_column="ID", primary_key=True)
    # Field name made lowercase.
    diaabertoid = models.ForeignKey(Diaaberto, models.CASCADE, db_column="DiaAbertoID")
    # Field name made lowercase.
    nome = models.CharField(db_column="Nome", max_length=255, blank=True, null=True)
    # Field name made lowercase.
...
class Unidadeorganica(models.Model):
    ...
    sigla = models.CharField(
        db_column="Sigla", max_length=255
    )  # Field name made lowercase.
