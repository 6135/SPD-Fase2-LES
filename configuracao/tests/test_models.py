...
def create_curso(uo):
    return Curso.objects.create(
        nome="Licensiatura Engenharia Informatica", sigla="LEI", unidadeorganicaid=uo
    )


def create_horario(inicio=time(11, 0), fim=time(11, 30)):
    return Horario.objects.create(inicio=inicio, fim=fim)


def create_menu(campus, horario, diaaberto):
    return Menu.objects.create(
        horarioid=horario, campus=campus, diaaberto=diaaberto, dia=date(1970, 1, 1)
    )


def create_prato(menu):
    return Prato.objects.create(
        prato="Feijoada", tipo="Carne", nrpratosdisponiveis=20, menuid=menu
    )


def create_edificio(campus):
    return Edificio.objects.create(
        nome="C1", campus=campus, image="images/edifi/gambelas.jpg"
    )


def create_sala(edificio):
    return Espaco.objects.create(
        nome="2.13", edificio=edificio, andar="0", descricao="Uma sala normal"
    )


def create_transporteH(diaaberto):
    transporte = Transporte.objects.create(
        identificador="01-00", diaaberto=diaaberto, dia=date(1970, 1, 1)
    )
...
def create_transporteU(transporte):
    return Transporteuniversitario.objects.create(transporte=transporte, capacidade=20)
...
class TestModels(TestCase):
    ...
    def test_dia_aberto(self):
        ...
        self.assertEquals(
            diaaberto.datadiaabertoinicio,
            datetime(1970, 1, 1, 9, 30, tzinfo=pytz.UTC),
        )
        self.assertEquals(
            diaaberto.datadiaabertofim, datetime(2040, 1, 2, 9, 30, tzinfo=pytz.UTC)
        )
        self.assertEquals(
            diaaberto.datainscricaoatividadesinicio,
            datetime(1970, 1, 1, 9, 30, tzinfo=pytz.UTC),
        )
        self.assertEquals(
            diaaberto.datainscricaoatividadesfim,
            datetime(2040, 1, 2, 9, 30, tzinfo=pytz.UTC),
        )
        self.assertEquals(
            diaaberto.datapropostasatividadesincio,
            datetime(1970, 1, 1, 9, 30, tzinfo=pytz.UTC),
        )
        self.assertEquals(
            diaaberto.dataporpostaatividadesfim,
            datetime(2040, 1, 2, 9, 30, tzinfo=pytz.UTC),
        )
    ...
    def test_dep(self):
        ...
        self.assertEquals(
            dep.nome, "Departamento de Engenharia Informatica e Eletronica"
        )
    ...
    def test_menu(self):
        ...
        self.assertEquals(
            menu.pratos_().first().id, Prato.objects.filter(menuid=menu).first().id
        )
    ...
    def test_edificio(self):
        ...
        self.assertEquals(
            str(edifi),
            "<a href='/configuracao/imagens/edificio/" + str(edifi.id) + "'>C1</a>",
        )
