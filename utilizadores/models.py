...
class ProfessorUniversitario(Utilizador):
    ...
    faculdade = models.ForeignKey("configuracao.Unidadeorganica", models.CASCADE)

    departamento = models.ForeignKey("configuracao.Departamento", models.CASCADE)
...
class Colaborador(Utilizador):
    curso = models.ForeignKey("configuracao.Curso", models.CASCADE)
    ...
    def get_free_colabs(coord, dia, horario, tipo, sessao=None):
        ...
        colabs = Colaborador.objects.filter(
            faculdade=coord.faculdade, utilizador_ptr_id__valido=True
        )
        free = True
        for colab in colabs:
            tarefas = coordmodels.Tarefa.objects.filter(
                colab=colab.id, horario=horario, dia=dia
            )
            ...
            if not ColaboradorHorario.objects.filter(colab=colab.id).exists():
            ...
            elif sessao is not None and (
                Preferencia.objects.filter(
                    colab=colab.id, tipoTarefa="tarefaAuxiliar"
                ).exists()
                or not Preferencia.objects.filter(colab=colab.id).exists()
            ):
                ...
                if (
                    not ColaboradorHorario.objects.filter(colab=colab.id, dia=dia)
                    .filter(inicio__lte=horario, fim__gte=horario)
                    .exists()
                ):
                    continue

                if (
                    coordmodels.Tarefa.objects.filter(
                        colab=colab.id, dia=dia, horario__gt=inicio
                    )
                    .filter(horario__lt=fim)
                    .exists()
                ):
                    continue
                if coordmodels.TarefaAuxiliar.objects.filter(
                    tarefaid__colab=colab.id,
                    tarefaid__dia=dia,
                    sessao__horarioid__inicio__lt=inicio,
                    sessao__horarioid__fim__gt=inicio,
                ).exists():
                    continue
                if (
                    PreferenciaAtividade.objects.filter(
                        preferencia__colab=colab.id
                    ).exists()
                    and not PreferenciaAtividade.objects.filter(
                        preferencia__colab=colab.id, atividade=s.atividadeid.id
                    ).exists()
                ):
                    continue
                ...
                for t in tarefas:
                    ...
                    if t.tipo != "tarefaAuxiliar" and (
                        datetime.strptime(str(t.horario), "%H:%M:%S") - hinicio
                        > timedelta(hours=0, minutes=15, seconds=0)
                        or hfim - datetime.strptime(str(t.horario), "%H:%M:%S")
                        < timedelta(days=-1, hours=23, minutes=45)
                    ):
                        free = False
                if free == True:
                ...
                if not Preferencia.objects.filter(
                    colab=colab.id, tipoTarefa=tipo
                ).exists():
                    continue
                ...
                for t in tarefas:
                    ...
                    if datetime.strptime(str(t.horario), "%H:%M:%S") - h > timedelta(
                        days=-1, hours=23, minutes=45
                    ) and h - datetime.strptime(str(t.horario), "%H:%M:%S") < timedelta(
                        minutes=15
                    ):
                        free = False
                if (
                    coordmodels.TarefaAuxiliar.objects.filter(
                        tarefaid__colab=colab.id, tarefaid__dia=dia
                    )
                    .filter(
                        sessao__horarioid__inicio__lte=horario,
                        sessao__horarioid__fim__gte=horario,
                    )
                    .exists()
                ):
                    continue
    ...
    def get_preferencia_atividade(self):
        ...
        for horario in horarios:
            sessoes = amodels.Sessao.objects.filter(
                horarioid__fim__gte=horario.inicio,
                horarioid__fim__lte=horario.fim,
                atividadeid__estado="Aceite",
                atividadeid__professoruniversitarioutilizadorid__faculdade=self.faculdade,
                dia=horario.dia,
            )
    ...
    def get_tarefas(self, dia, inicio, fim):
        if (
            len(
                coordmodels.Tarefa.objects.filter(
                    colab=self, horario__gte=inicio, horario__lte=fim, dia=dia
                )
            )
            > 0
        ):
            ...
            len(
                coordmodels.TarefaAuxiliar.objects.filter(
                    tarefaid__colab=self,
                    sessao__horarioid__gte=inicio,
                    sessao__horarioid__lte=fim,
                    sessao__dia=dia,
                )
            )
            > 0
