...
def atualizar_informacoes(user):
    if user.is_authenticated:
        ...
        info = InformacaoNotificacao.objects.filter(recetor=utilizador_recetor)
        for x in info:
            try:
...
def nr_mensagens(user):
    if user.is_authenticated:
        msg = MensagemRecebida.objects.select_related("mensagem__recetor").filter(
...
def nr_mensagens_nao_lidas(user):
    if user.is_authenticated:
        msg = MensagemRecebida.objects.select_related("mensagem__recetor").filter(
            mensagem__recetor=user.id, mensagem__lido=False
        )
        return len(msg)
    else:
        return 0
