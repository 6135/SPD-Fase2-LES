    ...
    def __init__(
        ...
        if attrs is not None:
            ...
            if hours and minutes is not None:
                self.attrs = {
                    "class": "input",
                    "value": str(now.date()) + " " + hours + ":" + minutes,
                }
            else:
                self.attrs = {
                    "class": "input",
                    "value": str(now.date()) + " " + str(now.time().strftime("%H:%M")),
                }
...
class diaAbertoSettingsForm(ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            ...
            "precoalunos": NumberInput(
                attrs={"class": "input", "step": "0.01", "min": "0"}
            ),
            "precoprofessores": NumberInput(
                attrs={"class": "input", "step": "0.01", "min": "0"}
            ),
            "escalasessoes": TimeInput(
                attrs={
                    "class": "input",
                    "step": "00:05",
                    "min": "00:05",
...
class menuForm(ModelForm):
    ...
    def clean(self):
        ...
        if campus_data is not None and diaaberto_data is not None:
            ...
            self.instance.horarioid = Horario.objects.get(
                inicio="12:00:00", fim="14:00:00"
            )
        else:
...
class menusFilterForm(Form):
    searchAno = CharField(
        widget=TextInput(attrs={"class": "input", "placeholder": "Ano"}), required=False
    )
...
class transporteFilterForm(Form):
    ...
    filter_from = ChoiceField(
        choices=[
            (None, "De"),
            ("Gambelas", "De Gambelas"),
            ("Penha", "De Penha"),
    ...
    filter_to = ChoiceField(
        choices=[
            (None, "Para"),
            ("Gambelas", "Para Gambelas"),
            ("Penha", "Para Penha"),
...
class transporteUniversitarioForm(ModelForm):
    class Meta:
        ...
        widgets = {"capacidade": TextInput(attrs={"class": "input is-half"})}
