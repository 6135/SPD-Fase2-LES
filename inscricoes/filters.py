...
def filter_nome_responsavel(queryset, name, value):
    return queryset.filter(
        Exists(
            Responsavel.objects.filter(inscricao=OuterRef("pk"), nome__icontains=value)
        )
    )
...
class InscricaoFilter(django_filters.FilterSet):
    ...
    min_alunos = django_filters.NumberFilter(field_name="nalunos", lookup_expr="gte")
    max_alunos = django_filters.NumberFilter(field_name="nalunos", lookup_expr="lte")
    departamento = django_filters.CharFilter(method=filter_departamento)
    participante = django_filters.CharFilter(method=filter_nome_responsavel)
