...
class InscricaoForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        if not cleaned_data.get("diaaberto", ""):
            ...
            if prox_diaaberto:
                raise ValidationError(
                    _(
                        f"""A data que escolheu não faz parte do Dia Aberto. Próximo dia aberto: de {prox_diaaberto.datadiaabertoinicio.strftime("%d/%m/%Y às %H:%M")}, até {prox_diaaberto.datadiaabertofim.strftime("%d/%m/%Y às %H:%M")}."""
                    )
                )
            else:
                raise ValidationError(
        ...
        if not cleaned_data.get("individual", False) and (
            not cleaned_data.get("ano", False)
            or not cleaned_data.get("turma", False)
            or not cleaned_data.get("areacientifica", False)
        ):
            raise ValidationError(_("Por favor, introduza toda a informação da turma."))
        if self.instance:
            inscricaoprato = self.instance.inscricaoprato_set.first()
            if (
                inscricaoprato
                and inscricaoprato.npratosalunos + inscricaoprato.npratosdocentes
                > cleaned_data.get("nalunos", 0) + 5
            ):
                raise ValidationError(
                    _(
                        "As inscrições nos almoços excedem o número de participantes disponíveis."
                    )
                )
            ...
            for inscricao_sessao in inscricoes_sessao:
                if inscricao_sessao.nparticipantes > cleaned_data.get("nalunos", 0):
                    raise ValidationError(
                        _(
                            "As inscrições nas sessões excedem o número de participantes disponíveis."
                        )
                    )

    def save(self, commit=True):
        self.instance.escola = models.Escola.objects.get_or_create(
            nome=self.cleaned_data["nome_escola"], local=self.cleaned_data["local"]
        )[0]
...
class TransporteForm(forms.Form):
    ...
    def clean(self):
        ...
        if cleaned_data.get("meio", "") != "outro" and (
            not cleaned_data.get("hora_chegada", False)
            or not cleaned_data.get("local_chegada", False)
        ):
...
class AlmocoForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        if (
            cleaned_data.get("campus", False)
            and cleaned_data.get("npratosalunos", 0) <= 0
            and cleaned_data.get("npratosdocentes", 0) <= 0
        ):
            raise forms.ValidationError(
                _(
                    "Por favor, indique o número de pessoas se pretendem almoçar num Campus."
                )
            )
        if (
            cleaned_data.get("npratosalunos", 0) > 0
            or cleaned_data.get("npratosdocentes", 0) > 0
        ) and not cleaned_data.get("campus", False):
            raise forms.ValidationError(
                _(
                    "Por favor, indique o Campus se 1 ou mais pessoas pretendem almoçar na Universidade."
                )
            )
        if not cleaned_data.get("individual", False):
            if cleaned_data.get("npratosalunos", 0) > cleaned_data.get("nalunos", 0):
                raise forms.ValidationError(
                    _(
                        "O número de alunos inscritos no almoço excede o número de alunos disponíveis"
                    )
                )
            if (
                cleaned_data.get("npratosdocentes", 0)
                > cleaned_data.get("nresponsaveis", 0) + 5
            ):
                raise forms.ValidationError(
                    _(
                        "O número de docentes inscritos no almoço excede o número de docentes disponíveis"
                    )
                )
        else:
            if cleaned_data.get("npratosalunos", 0) + cleaned_data.get(
                "npratosdocentes", 0
            ) > cleaned_data.get("nalunos", 0):
                raise forms.ValidationError(
                    _(
                        "O número de inscritos no almoço excede o número de pessoas disponíveis"
                    )
                )

    def save(self, commit=True):
        if self.cleaned_data.get("campus", False) and (
            self.cleaned_data.get("npratosalunos", 0) > 0
            or self.cleaned_data.get("npratosdocentes", 0) > 0
        ):
...
def verificar_vagas(sessoes, nalunos, dia):
    ...
    for sessao in sessoes:
        try:
            ...
            raise forms.ValidationError(
                _(
                    "Ocorreu um erro inesperado. Por favor, tente submeter uma nova inscrição."
                )
            )
    for sessao in sessoes:
        try:
        ...
        if sessao_obj.atividadeid.estado != "Aceite":
            raise forms.ValidationError(
                _(
                    f'A seguinte atividade não se encontra validada: "{sessao_obj.atividadeid.nome}".'
                )
            )
        if sessao_obj.dia != dia:
            raise forms.ValidationError(
                _(
                    f"A seguinte sessão não faz parte do dia da inscrição: \"{sessao_obj.atividadeid.nome}\", dia {sessao_obj.dia}, das {sessao_obj.horarioid.inicio.strftime('%H:%M')} às {sessao_obj.horarioid.fim.strftime('%H:%M')}. Dia da inscrição: {dia}"
                )
            )
        if sessoes[sessao] > sessao_obj.vagas:
            raise forms.ValidationError(
                _(
                    f"O número de inscritos na sessão da atividade \"{sessao_obj.atividadeid.nome}\", das {sessao_obj.horarioid.inicio.strftime('%H:%M')} às {sessao_obj.horarioid.fim.strftime('%H:%M')} ({sessoes[sessao]} inscritos) excede o nº de vagas para essa sessão ({sessao_obj.vagas} vagas). Este erro pode ter ocorrido porque foi submetida entretanto uma outra inscrição que ocupou as vagas pretendidas. Por favor, atualize as suas inscrições nas sessões."
                )
            )
        if sessoes[sessao] > nalunos:
            raise forms.ValidationError(
                _(
                    f"O número de inscritos na sessão da atividade \"{sessao_obj.atividadeid.nome}\", das {sessao_obj.horarioid.inicio.strftime('%H:%M')} às {sessao_obj.horarioid.fim.strftime('%H:%M')} ({sessoes[sessao]} inscritos) excede o nº de alunos disponíveis nesse horário ({nalunos} alunos)."
                )
            )
        sessoes_simultaneas = []
        for inscritos_horario in inscritos_horarios:
            if inscritos_horario["sessao"] is not sessao and horarios_intersetam(
                sessao_obj.horarioid.inicio,
                sessao_obj.horarioid.fim,
                inscritos_horario["inicio"],
                inscritos_horario["fim"],
            ):
...
class SessoesForm(forms.Form):
    ...
    def clean(self):
        ...
        try:
            cleaned_data["sessoes"] = cleaned_data.get("sessoes", "{}").replace(
                "\\", ""
            )
            pattern = re.compile(
            ...
            cleaned_data["sessoes"] = {
                sessao: _sessoes[sessao] for sessao in _sessoes if _sessoes[sessao] > 0
            }
        except:
            raise forms.ValidationError(
                _(
