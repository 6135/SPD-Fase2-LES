...
def min_date(diaaberto):
    try:
        return Diaaberto.objects.get(id=diaaberto).datadiaabertoinicio.strftime(
            "%d/%m/%Y"
        )
    except:
        return ""
...
def ninscricoesindividuais(diaaberto):
    result = diaaberto.inscricao_set.filter(individual=True).aggregate(Sum("nalunos"))
    return result["nalunos__sum"] if result["nalunos__sum"] is not None else 0


@register.filter
def ninscricoesescola(diaaberto):
    result = diaaberto.inscricao_set.filter(individual=False).aggregate(Sum("nalunos"))
    return result["nalunos__sum"] if result["nalunos__sum"] is not None else 0
...
def atividadesdepartamento(departamento):
    result = Atividade.objects.filter(
        professoruniversitarioutilizadorid__departamento=departamento, estado="Aceite"
    ).count()
    return result
...
def sessoesdia(dia):
    ...
    result = Sessao.objects.filter(dia=dia, atividadeid__estado="Aceite").count()
    return result
...
def inscritossessoesdia(dia):
    ...
    result = Inscricaosessao.objects.filter(
        sessao__dia=dia, sessao__atividadeid__estado="Aceite"
    ).aggregate(Sum("nparticipantes"))
    return (
        result["nparticipantes__sum"]
        if result["nparticipantes__sum"] is not None
        else 0
    )
...
def transportesdia(dia):
    dia = datetime.strptime(dia, "%d/%m/%Y")
    result = (
        Inscricao.objects.filter(dia=dia)
        .filter(
            ~Q(meio_transporte="outro"),
        )
        .aggregate(Sum("nalunos"))
    )
    return result["nalunos__sum"] if result["nalunos__sum"] is not None else 0
...
def almocosalunosdia(dia):
    dia = datetime.strptime(dia, "%d/%m/%Y")
    result = Inscricaoprato.objects.filter(inscricao__dia=dia).aggregate(
        Sum("npratosalunos")
    )
    return (
        result["npratosalunos__sum"] if result["npratosalunos__sum"] is not None else 0
    )
...
def almocosdocentesdia(dia):
    ...
    return (
        result["npratosdocentes__sum"]
        if result["npratosdocentes__sum"] is not None
        else 0
    )
...
def almocampenha(diaaberto):
    ...
    result = (
        alunos["npratosalunos__sum"] if alunos["npratosalunos__sum"] is not None else 0
    )
    result += (
        docentes["npratosdocentes__sum"]
        if docentes["npratosdocentes__sum"] is not None
        else 0
    )
    return result
