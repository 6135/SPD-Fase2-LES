...
def sem_notificacoes(request, id):
    ...
    return render(
        request,
        "notificacoes/sem_notificacoes.html",
        {
            "categoria": id,
        },
    )
...
def categorias_notificacao_automatica(request, id, nr):
    ...
    if id == 1:
        ...
        notificacoes = Notificacao.objects.filter(
            recipient_id=user, public=False
        ).order_by("-id")
    elif id == 4:
        notificacoes = Notificacao.objects.filter(
            recipient_id=user, public=True
        ).order_by("-id")
    elif id == 5:
        notificacoes = Notificacao.objects.filter(
            recipient_id=user, level="info"
        ).order_by("-id")
    elif id == 6:
        notificacoes = Notificacao.objects.filter(
            recipient_id=user, level="warning"
        ).order_by("-id")
    elif id == 7:
        notificacoes = Notificacao.objects.filter(
            recipient_id=user, level="error"
        ).order_by("-id")
    elif id == 8:
        notificacoes = Notificacao.objects.filter(
            recipient_id=user, level="success"
        ).order_by("-id")
    else:
    ...
    return render(
        request,
        "notificacoes/detalhes_notificacao_automatica.html",
        {
            "atual": notificacao,
...
def enviar_notificacao_automatica(request, sigla, id):
    ...
    if sigla == "cancelarTarefa":
        ...
        descricao = (
            'Foi feito um pedido de cancelamento da tarefa "'
    ...
    # Enviar notificação ao enviar rejeicao do pedido de cancelamento da tarefa - coordenador
        ...
        descricao = (
            'O pedido de cancelamento da tarefa "'
            + tarefa.getDescription()
            + '" foi rejeitado.'
        )
        user_recipient = Utilizador.objects.get(id=tarefa.colab.id)
    ...
    # Enviar notificação atividade apagada - coordenador
        ...
        user_recipient = Utilizador.objects.get(id=atividade.get_coord().id)
        notify.send(
            sender=user_sender,
            recipient=user_recipient,
            verb=descricao,
    ...
    # Enviar notificação quando há registo de utilizador por validar - administrador e ao coordenador ( 5 dias depois de criado se ainda tiver pendente
        ...
        for x in administradores:
            ...
            info = InformacaoNotificacao(
                data=timezone.now() + timedelta(days=5),
                pendente=True,
                titulo=titulo,
                descricao=descricao,
                emissor=user_sender,
                recetor=user_recipient,
                tipo="register " + str(id),
                lido=False,
            )
            info.save()
        if user_sender.getProfile() != "Administrador":
            coordenadores = Coordenador.objects.filter(
                faculdade=Unidadeorganica.objects.get(
                    id=user_sender.getUser().faculdade.id
                )
            )
...
def criar_mensagem(request, id):
    ...
    if request.user.is_authenticated:
        ...
        if (
            user.getProfile() == "Coordenador"
            or user.getProfile() == "Colaborador"
            or user.getProfile() == "ProfessorUniversitario"
        ):
        ...
        if form.is_valid():
            ...
            if tipo == 0:
                ...
                for x in utilizadores:
                    user_recipient = Utilizador.objects.get(
                        user_ptr_id=x.utilizador_ptr_id
                    )
                    info = InformacaoMensagem(
                        data=timezone.now(),
                        pendente=True,
                        ...
                        emissor=user,
                        recetor=user_recipient,
                        tipo="Grupo de admistradores do dia aberto",
                        lido=False,
                    )
                    info.save()
            ...
            if tipo == 0:
                ...
                if tipo_utilizadores == "ProfessorUniversitario":
                    utilizadores = ProfessorUniversitario.objects.filter(
                        faculdade=user.getUser().faculdade
                    )
...
def limpar_mensagens(request, id):
    ...
    if id == 5:
        notificacoes = MensagemEnviada.objects.select_related(
            "mensagem__emissor"
        ).filter(mensagem__emissor=user.id)
    elif id == 1:
        notificacoes = MensagemRecebida.objects.select_related(
            "mensagem__recetor"
        ).filter(mensagem__recetor=user.id, mensagem__lido=False)
    elif id == 2:
        notificacoes = MensagemRecebida.objects.select_related(
            "mensagem__recetor"
        ).filter(mensagem__recetor=user.id, mensagem__lido=True)
    elif id == 3:
        notificacoes = MensagemRecebida.objects.select_related(
            "mensagem__recetor"
        ).filter(mensagem__recetor=user.id, mensagem__pendente=False)
    elif id == 4:
        notificacoes = MensagemRecebida.objects.select_related(
            "mensagem__recetor"
        ).filter(mensagem__recetor=user.id, mensagem__pendente=True)
    else:
        notificacoes = MensagemRecebida.objects.select_related(
            "mensagem__recetor"
        ).filter(mensagem__recetor=user.id)
...
def mensagem_como_lida(request, id):
    ...
    if request.user.is_authenticated:
        ...
        return redirect("utilizadores:mensagem", 5)
    msgs = MensagemRecebida.objects.select_related("mensagem__recetor").filter(
        mensagem__recetor=user.id
    )
    for msg in msgs:
...
def sem_mensagens(request, id):
    ...
    return render(
        request,
        "notificacoes/sem_mensagens.html",
        {
            "categoria": id,
        },
    )
...
def detalhes_mensagens(request, id, nr):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            info = InformacaoMensagem(
                ...
                descricao=mensagem,
                emissor=user_sender,
                recetor=notificacao.mensagem.emissor,
                tipo="Individual",
                lido=False,
            )
            info.save()
    ...
    if id == 1:
        notificacoes = (
            MensagemRecebida.objects.select_related("mensagem__recetor")
            .filter(mensagem__recetor=user.id, mensagem__lido=False)
            .order_by("-id")
        )
    elif id == 2:
        ...
        notificacoes = (
            MensagemEnviada.objects.select_related("mensagem__emissor")
            .filter(mensagem__emissor=user.id)
            .order_by("-id")
        )
    else:
    ...
    return render(
        request,
        "notificacoes/detalhes_mensagens.html",
        {
            "form": form,
            "atual": notificacao,
            "notificacoes": notificacoes,
