...
def get_dias():
    try:
        ...
        diaaberto = Diaaberto.objects.get(
            datadiaabertoinicio__gte=today, datadiaabertofim__gte=today
        )
        ...
        return [("", "Escolha o dia")] + [
            (diainicio + timedelta(days=d), diainicio + timedelta(days=d))
...
def get_atividades_choices(fac):
    return [("", "Escolha a Atividade")] + [
        (atividade.id, atividade.nome)
        for atividade in Atividade.tarefas_get_atividades(fac)
    ]
...
class TarefaAuxiliarForm(Form):
    ...
    def __init__(self, facul, *args, **kwargs):
        ...
        self.fields["atividade"] = ChoiceField(
            widget=Select(attrs={"onchange": "diasSelect();"}),
            choices=get_atividades_choices(facul),
        )
    ...
    def save(self, user, id=None):
        ...
        if id is None:
            tarefa = Tarefa.objects.create(
                ...
                colab=colab,
                dia=data.get("dia"),
                horario=sessao.horarioid.inicio,
            )
            TarefaAuxiliar.objects.create(tarefaid=tarefa, sessao=sessao)
            return tarefa.id
        else:
            Tarefa.objects.filter(id=id).update(
                nome=nome,
                estado=estado,
                coord=user,
                colab=colab,
...
class TarefaAcompanharForm(Form):
    grupo = ChoiceField(
        widget=Select(
            attrs={
                "onchange": "diasGrupo();grupoInfo();grupoHorario();grupoOrigem();grupoDestino();"
            }
        )
    )
    ...
    def __init__(self, facul, *args, **kwargs):
        ...
        self.fields["grupo"] = ChoiceField(
            widget=Select(
                attrs={
                    "onchange": "diasGrupo();grupoInfo();grupoHorario();grupoOrigem();grupoDestino();"
                }
            ),
            choices=get_inscricao_choices(facul),
        )
    ...
    def save(self, user, id):
        ...
        if id is None:
            ...
            TarefaAcompanhar.objects.create(
                tarefaid=tarefa, origem=origem, destino=str(destino.id), inscricao=grupo
            )
            return tarefa.id
        else:
            Tarefa.objects.filter(id=id).update(
            ...
            TarefaAcompanhar.objects.filter(tarefaid=id).update(
                origem=origem, destino=str(destino.id), inscricao=grupo
            )
            return id
...
class TarefaOutraForm(Form):
    dia = ChoiceField(
        widget=Select(attrs={"onchange": "colaboradoresSelect()"}), choices=get_dias()
    )
    horario = TimeField(
        widget=TimeInput(
            attrs={
    ...
    def save(self, user, id=None):
        ...
        if id is None:
            ...
            TarefaOutra.objects.filter(tarefaid=id).update(
